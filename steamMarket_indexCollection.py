...
cookie = {'steamLoginSecure': 'Replace this string with your cookie'}
...
def requests_handler(alocatedItemNames):
    ...
    for currItem in alocatedItemNames: # go through all item names
        ...
        print(str(currRun),' out of ',str(len(alocatedItemNames))+' code: '+str(item.status_code)) 
...
def threads_exec(gameID):
    ...
    with open('data/'+gameID+'ItemNames_4.txt', "rb") as file:   # Unpickling
